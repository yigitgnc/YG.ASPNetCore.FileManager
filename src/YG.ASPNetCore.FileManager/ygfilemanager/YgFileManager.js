...
class YgFileManager {
    ...
    constructor(id, apiEndPointUrl, config) {
        ...
        YgFileManager.instances.push(this);
    }

    destroy() {
        let i = YgFileManager.instances.indexOf(this);
        YgFileManager.instances.splice(i, 1);
    }

    static selectById(id) {
        if (id && YgFileManager.instances && YgFileManager.instances.length > 0) {
            let instance = YgFileManager.instances.filter(function (el) {
                return el && el.thisId == "yg_fm_" + id;
            });
    ...
    #initMenu() {
        ...
        menuContinerDiv.classList.add('yg-fm-menu');
    ...
    #initExplorer() {
        ...
        this.#fileManager().classList.add('yg-fm-wrapper');
        ...
        wrapper.classList.add('yg-fm-content-wrapper');
        this.#fileManager().appendChild(wrapper);

        let jsTreeDiv = document.createElement('div');
        jsTreeDiv.classList.add('yg-fm-tree');
        wrapper.appendChild(jsTreeDiv);

        this.jsSelectContinerDiv = document.createElement('div');
        this.jsSelectContinerDiv.classList.add('yg-fm-fsitems');
        if (Cookies.get('yg-fm-current-view-' + this.thisId))
            this.jsSelectContinerDiv.classList.add(Cookies.get('yg-fm-current-view-' + this.thisId));
        if (!disabledFunctions.includes('browse')) 
        ...
        this.breadcrumbContinerDiv.classList.add('yg-fm-breadcrumb');
        ...
        this.jsSelect = new SelectionArea({
            selectables: ['#' + this.thisId + ' .yg-fm-fsitems > .fsitem'],
            boundaries: [this.jsSelectContinerDiv],
        }).on('beforestart', ({ store, event }) => {
            let contextMenu = $('#' + this.thisId + ' .yg-fm-fsitems .context');
    ...
    #fillFolderContent(data) {
        ...
        $.each(data.Folders, function (idx, folder) {
            ...
            fsItemImg.src = '/ygfilemanager/images/loading.gif';
            fsItemImg.setAttribute('data-src', '/ygfilemanager/images/folder.png');
        ...
        data.Files.forEach((file) => {
            ...
            if (!disabledFunctions.includes('filepreview')) {
                ...
                fsItemImg.setAttribute('data-src', '/ygfilemanager/images/file.png');
            }
            fsItemDiv.appendChild(fsItemImg);
    ...
    #setCurrentPath(path) {
        if (path)
            Cookies.set('yg-fm-current-path-' + this.thisId, path, { expires: 365 });
    }

    getCurrentPath = function () {
        let path = Cookies.get('yg-fm-current-path-' + this.thisId);
        if (path)
            return path.replace(/\\+$/, '') + "\\";
        return "";
    }
    ...
    toggleFilesListView = function () {
        if (this.jsSelectContinerDiv.classList.contains('list')) {
            ...
            Cookies.set('yg-fm-current-view-' + this.thisId, '', { expires: 365 });
        }
        else {
            this.jsSelectContinerDiv.classList.add('list');
            Cookies.set('yg-fm-current-view-' + this.thisId, 'list', { expires: 365 });
        }
    }

    showUploadPanel = function () {
        let uploadPanel = document.createElement('div');
        uploadPanel.classList.add('yg-upload-panel', 'dropzone');
